 ...
 */
public class ForAll extends AbstractExpression {
    Variable var;
    Expression expr;
    ...
    public Pair<String, Expression> arithmeticMatcherRec(Expression expr, Set<String> bound) throws ArithmeticMatcherException {
        if ((expr instanceof ForAll) && (var.hashCode() == ((ForAll)expr).getVar().hashCode())) {
            boolean wasBound = !bound.add(var.toString());
            Pair<String, Expression>  subExpr;
            try {
                subExpr = this.expr.arithmeticMatcherRec(((ForAll) expr).getExpr(), bound);
    ...
    public Expression clone() {
        return new ForAll((Variable) var.clone(), expr.clone());
    }
