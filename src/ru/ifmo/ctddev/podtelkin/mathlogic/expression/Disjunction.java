...
public class Disjunction extends AbstractExpression {
    ...
    public boolean matchSchemaRec(Expression schema, Map<String, Integer> mapper) {
        if (schema instanceof Disjunction) {
            return A.matchSchemaRec(((Disjunction)schema).getA(), mapper) &&
                    B.matchSchemaRec(((Disjunction)schema).getB(), mapper);
        }
    ...
    public Pair<String, Expression> arithmeticMatcherRec(Expression expr, Set<String> bound) throws ArithmeticMatcherException {
        if (expr instanceof Disjunction) {
            Pair<String, Expression>  u = A.arithmeticMatcherRec(((Disjunction)expr).getA(), bound);
            Pair<String, Expression>  v = B.arithmeticMatcherRec(((Disjunction)expr).getB(), bound);
    ...
    public Expression substituteAll(Map<String, Expression> substitutions, Set<String> bound) throws SubstitutionException {
        return new Disjunction(A.substituteAll(substitutions, bound), B.substituteAll(substitutions, bound));
    }
